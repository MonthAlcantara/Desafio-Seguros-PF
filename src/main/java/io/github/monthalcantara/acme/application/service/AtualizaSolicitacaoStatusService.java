...
public class AtualizaSolicitacaoStatusService {
    ...
    public AtualizaSolicitacaoStatusService(final SolicitacaoRepository solicitacaoRepository) {
    ...
    public void atualizarStatusComBaseEmFraude(final UUID solicitacaoId, final FraudCheckResponse response) {
        ...
        solicitacaoRepository.findById(solicitacaoId).ifPresent(solicitacao -> {
            final var classificacao = ClassificacaoFraude.fromDescricao(response.getClassificacao());
            final var novoStatus = classificacao.getStatus();
